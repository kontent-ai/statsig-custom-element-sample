...
export const handleCorsRequests = (event: HandlerEvent, allowedMethods: ReadonlyArray<string>): HandlerResponse | null => {
...
export const expectEnvVars = <VarNames extends ReadonlyArray<string>>(allowedMethods: ReadonlyArray<string>, varNames: VarNames): Readonly<{ success: true; result: VarValues<VarNames> } | { success: false; response: HandlerResponse }> => {
  ...
  if (missingVars.length) {
    return {
      ...
      response: {
        statusCode: 500,
        headers: createCorsHeaders(allowedMethods),
        body: JSON.stringify({ error: `Environment variables ${missingVars.join(", ")} are not configured.`}),
      },
    };
  }
...
type VarValues<VarNames extends ReadonlyArray<string>> = { readonly [key in keyof VarNames]: string };
