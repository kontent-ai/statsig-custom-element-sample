import { ContentTypeModels, ElementModels, LanguageVariantElements, LanguageVariantModels, ManagementClient } from '@kontent-ai/management-sdk';
import { type IDeliveryClient } from '@kontent-ai/delivery-sdk';
import type { ExperimentScenario, ComponentSearchResult } from '../../../src/types';

const withErrorCatch = (scenario: string) => <Res extends Unpromisify<Result<unknown>>, Args extends unknown[]>(fn: (...args: Args) => Promise<Res>) => async (...args: Args): Promise<Res> => {
  try {
...
export const findExperimentUsages = withErrorCatch('finding usages')(async (
  client: IDeliveryClient,
  itemCodename: string
): Result<ReadonlyArray<UsageInfo>> => {
...
export const getWinningVariantItems = withErrorCatch('getting winning variant items')(async (
...
export const replaceExperimentReference = withErrorCatch('replacing experiment reference')(async (
  ...
  const updatedElements = variant.elements.map((element) => {
    ...
    if (typeof value === 'string' && value.includes(experimentItemId)) {
      const winningItemsHtml = winningItemIds
        .map((id) => `<object type="application/kenticocloud" data-type="item" data-id="${id}"></object>`)
        .join('');
...
export const deleteExperimentItem = withErrorCatch('deleting experiment item')(async (
  client: ManagementClient,
  experimentItemId: string
): Result<null> => client
  .deleteContentItem()
...
const isExperimentComponent = (
  ...
  const statsigElement = component.elements.find(
...
export const replaceComponentWithWinningVariant = withErrorCatch('replacing component with winning variant')(async (
  client: ManagementClient,
  variant: LanguageVariantModels.ContentItemLanguageVariant,
  parentItemId: string,
  componentId: string,
...
export const findComponentWithExperiment = withErrorCatch('finding component with experiment')(async (
  variant: LanguageVariantModels.ContentItemLanguageVariant,
  experimentType: ContentTypeModels.ContentType,
  experimentId: string,
  winningVariant: 'control' | 'test',
): Result<ComponentSearchResult | null> => {
  const experimentCustomElementId = experimentType.elements.find((el) => el.codename === 'statsig_a_b_testing')?.id ?? '';
  const experimentWinningVariantElementId = experimentType.elements.find((el) => el.codename === winningVariant)?.id ?? '';
...
export const determineExperimentScenario = withErrorCatch('determining experiment scenario')(async (
  client: ManagementClient,
  itemId: string,
  experimentId: string,
  ...
  const statsigElementId = typeResponse.data.elements.find(
    (el) => el.codename === 'statsig_a_b_testing'
  )?.id ?? '';
