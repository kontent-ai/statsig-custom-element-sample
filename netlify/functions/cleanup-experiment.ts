...
import { concludeExperiment } from './utils/statsig';
...
export const handler: Handler = async (event) => {
  ...
  const varsRes = expectEnvVars(allowedMethods, ["KONTENT_PREVIEW_API_KEY", "KONTENT_MANAGEMENT_API_KEY", "STATSIG_CONSOLE_KEY"]);
  if (!varsRes.success) {
...
const performCleanup = async (
  ...
  if (scenarioResult.result.type === 'linked_item') {
    return performLinkedItemCleanup(
      body,
      managementClient,
      deliveryClient
    );
  ...
  return performComponentCleanup(
    body,
    managementClient,
    scenarioResult.result.parentItemId
  );
};
...
const performLinkedItemCleanup = async (
  ...
  const usagesResult = await findExperimentUsages(
    deliveryClient,
    body.experimentItemCodename
  );
  ...
  const winningItemsResult = await getWinningVariantItems(
    experimentVariant,
    body.winningVariant
  );
  ...
  const replaceErrors = replaceResults
    ...
    .filter(notNull);
  const replaced = replaceResults.map((res) => res.success ? res.parentCodename : null).filter(notNull);
...
const performComponentCleanup = async (
  ...
  if (!componentResult.success || !componentResult.result) {
    return {
      ...emptyResult,
      statsigConcluded: true,
      errors: [{ step: 'find_component', message: componentResult.success ? `Experiment component not found in parent item ${parentItemId}` : componentResult.error }],
    };
  }
