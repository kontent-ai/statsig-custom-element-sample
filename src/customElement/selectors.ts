...
export const getElement = async (elementCodename: string): Promise<ElementValue> => new Promise(resolve => {
...
export const useElements = (elementCodenames: ReadonlyArray<string>) => {
  ...
  const updateCodenames = useCallback(async (changedCodenames: ReadonlyArray<string>) =>
  ...
  useEffect(() => {
    CustomElement.observeElementChanges(elementCodenames, (codenames) => void updateCodenames(codenames));
  }, [elementCodenames, updateCodenames]);

  return watchedElements;
};

export const promptToSelectItems = async (params: Readonly<{ allowMultiple: boolean }>): Promise<ReadonlyArray<ItemDetail>> =>
...
export const promptToSelectAssets = async (params: Readonly<{ allowMultiple: boolean; fileType: "all" | "images" }>): Promise<ReadonlyArray<AssetDetail>> =>
...
const withFallback = <V>(fallbackValue: V) => (v: V | null) => v ?? fallbackValue;
